...
class Noise2Noise(object):
    ...
    def _compile(self):
        ...
        if self.trainable:
            ...
            #elif self.p.loss == 'dcp': #edited
             #   self.loss = DCLoss()
            else:
                self.loss = nn.L1Loss()
            self.loss2 = DCLoss() #edited
            self.loss3 = BCLoss() #edited
            #self.loss4 = GradLoss()

        # CUDA support
        print("is gpu avaliable??   " + str(torch.cuda.is_available()))#edited
        print("do we want to use cuda??   "+ str(self.p.cuda))#edited

        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        modal = UNet().to(device)
        summary(modal, (3, 64, 64))
        ...
        if self.use_cuda:
            ...
            if self.trainable:
                ...
                self.loss3 = self.loss3.cuda()
                #self.loss4 = self.loss4.cuda()
    ...
    def eval(self, valid_loader):
        ...
        for batch_idx, (source, target) in enumerate(valid_loader):
            ...
            # Update loss
            loss = 0.3333*(self.loss(source_denoised, target) + self.loss2(source_denoised, target) + self.loss3(source_denoised, target))#edited
    ...
    def train(self, train_loader, valid_loader):
        ...
        for epoch in range(self.p.nb_epochs):
            ...
            for batch_idx, (source, target) in enumerate(train_loader):
                ...
                loss = 0.333*(self.loss(source_denoised, target) + self.loss2(source_denoised, target) + self.loss3(source_denoised, target)) #edited
                loss_meter.update(loss.item())
...
class DCLoss(nn.Module): #edited dc loss
    """High dynamic range loss."""
    ...
    def forward(self, denoised, target):
        ...
        maxpool=maxpool.cuda()#edited

        dc = maxpool(1-denoised[:,None,:,:])
        target = torch.FloatTensor(dc.shape).zero_()
        target=target.to(device='cuda')#edited

        dcploss=nn.L1Loss(reduction='mean')
        dcploss=dcploss.cuda()#edited

        loss = alpha*dcploss(dc,target) 
        return torch.mean(loss.view(-1)) 

class BCLoss(nn.Module): #edited dc loss
    """High dynamic range loss."""
    ...
    def forward(self, denoised, target):
        ...
        bc = maxpool(denoised[:,None,:,:])
        target = torch.FloatTensor(bc.shape).zero_()
        target=target.to(device='cuda')#edited

        bcploss=nn.L1Loss(reduction='mean')
        bcploss=bcploss.cuda()#edited

        loss = alpha*bcploss(bc,target) 
        return torch.mean(loss.view(-1)) 


class GradLayer(nn.Module): #edited
    ...
    def __init__(self):
        ...
        kernel_h=kernel_h.to(device='cuda')#edited

        kernel_v = torch.FloatTensor(kernel_v).unsqueeze(0).unsqueeze(0)
        kernel_v=kernel_v.to(device='cuda')#edited

        self.weight_h = nn.Parameter(data=kernel_h, requires_grad=False)
        self.weight_h=self.weight_h.cuda()#edited

        self.weight_v = nn.Parameter(data=kernel_v, requires_grad=False)
        self.weight_v=self.weight_v.cuda()#edited
    ...
    def get_gray(self,x):
        ...
        convert=convert.to(device='cuda')#edited
    ...
    def forward(self, x):
        ...
        x=x.to(device='cuda')#edited

        return x

class GradLoss(nn.Module): #edited

    def __init__(self):
        super(GradLoss, self).__init__()
        self.loss = nn.L1Loss()
        self.loss=self.loss.cuda()#edited

        self.grad_layer = GradLayer()
        self.grad_layer=self.grad_layer.cuda()#edited
    ...
    ### print out dcp loss and mse loss seperately