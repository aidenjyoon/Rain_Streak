...
def load_dataset(root_dir, gt_dir, redux, params, shuffled=False, single=False):### also loads gt_dir even if not needed
    ...
    if single:
        dataset = DatasetTest(root_dir, gt_dir, redux, params.crop_size, ##added during 2.0
            clean_targets=params.clean_targets)
    elif params.noise_type == 'mc':
        dataset = MonteCarloDataset(root_dir, gt_dir, redux, params.crop_size,
            clean_targets=params.clean_targets)
    elif params.noise_type == 'none':   ### for clean input and ground truth comparisons
        dataset = RegDataset(root_dir, gt_dir, redux, params.crop_size, ##added during 2.0
            clean_targets=params.clean_targets)
    else:
        dataset = NoisyDataset(root_dir, gt_dir, redux, params.crop_size,
...
class AbstractDataset(Dataset):
    ...
    def __init__(self, root_dir, gt_dir, redux=0, crop_size=128, clean_targets=False): #edited during 2.0
        ...
        self.gt_dir = gt_dir ###added during 2.0
        self.redux = redux
    ...
    def _seeded_crop(self, img_list, i,j): ## added during 2.0
...
class NoisyDataset(AbstractDataset):
    ...
    def __init__(self, root_dir, gt_dir, redux, crop_size, clean_targets=False,
        noise_dist=('gaussian', 50.), seed=None):
        """Initializes noisy image dataset."""

        super(NoisyDataset, self).__init__(root_dir, gt_dir, redux, crop_size, clean_targets)
    ...
    ###########################################adding rain ###########################################################
    def _add_rain(self, img): ### adding rain
        ...
        ##### code starts
        drop_length=20; slant=0; drop_width=1; drop_color=(200,200,200); rain_count=int(self.noise_param) #initialzation statements
        imshape = np.shape(img)
        rain_streak_noise = np.zeros((imshape[0], imshape[1],imshape[2]))
        rain_drops = generate_random_lines(imshape, slant, drop_length, rain_count)### need code for this

        for rain_drop in rain_drops:
            cv2.line(rain_streak_noise,(rain_drop[0],rain_drop[1]),(rain_drop[0]+slant,rain_drop[1]+drop_length),drop_color,drop_width)
        
        rain_streak_noise= cv2.blur(rain_streak_noise,(5,5)) 

        noise_img = np.array(img + rain_streak_noise) ###np.array(img) + rain_streak_noise ## something went wrong here
        noise_img = np.clip(noise_img, 0, 255).astype(np.uint8)
        return Image.fromarray(noise_img)
    ###########################################adding rain ###########################################################
    ...
    def _corrupt(self, img):
        ...
        elif self.noise_type =='rain': ##if rain add rain
            return self._add_rain(img)
        else:
...
class RegDataset(AbstractDataset):    
    """Class for dealing with mannually inputed images."""
    def __init__(self, root_dir, gt_dir, redux, crop_size, clean_targets=False, ### check difference between two initialization and find what is needed and isnt
        noise_dist=('gaussian', 50.), seed=None):
        """Initializes noisy image dataset."""

        super(RegDataset, self).__init__(root_dir, gt_dir, redux, crop_size, clean_targets)
    ...
    def __getitem__(self, index): ### this will also run when class is ran so this is where we rehook the system
        """Retrieves image from folder and corrupts it."""

        # Load PIL image
        img_path = os.path.join(self.root_dir, self.imgs[index]) #this loads the image but we need 2 
        img =  Image.open(img_path).convert('RGB')
        gt_path = os.path.join(self.gt_dir, self.imgs[index])
        gt = Image.open(gt_path).convert('RGB')


        w, h = [img][0].size
        i = np.random.randint(0, h - self.crop_size + 1)
        j = np.random.randint(0, w - self.crop_size + 1) ### crop size?
        # Random square crop
        if self.crop_size != 0:
            img = self._seeded_crop([img],i,j)[0]
            gt = self._seeded_crop([gt],i,j)[0]

        # Corrupt source image
        # this line is prob not needed
        source = tvF.to_tensor(img) #source is the original, make it clean 

        # Corrupt target image, but not when clean targets are requested
        target = tvF.to_tensor(gt) # this line is more legit
            #target should also be clean (note this is same code as line 359-360
        return source, target
...
class DatasetTest(AbstractDataset):    
    ...
    def __init__(self, root_dir, gt_dir, redux, crop_size, clean_targets=False, ### check difference between two initialization and find what is needed and isnt
        ...
        super(DatasetTest, self).__init__(root_dir, gt_dir, redux, crop_size, clean_targets)
    ...
    def __getitem__(self, index): ### this will also run when class is ran so this is where we rehook the system
        ...
        w, h = [img][0].size
        i = int((h - self.crop_size)/2)
        j = int((w - self.crop_size)/2) ### crop size?
        # Random square crop
        if self.crop_size != 0:
            img = self._seeded_crop([img],i,j)[0]
            gt = self._seeded_crop([gt],i,j)[0]
